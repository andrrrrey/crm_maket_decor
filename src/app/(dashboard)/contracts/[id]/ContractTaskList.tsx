...
interface ContractTaskListProps {
  tasks: Task[];
  contractId: string;
  canEdit: boolean;
  type?: "task" | "purchase";
...
export function ContractTaskList({
  tasks,
  contractId,
  canEdit,
  type = "task",
  taskType,
}: ContractTaskListProps) {
  ...
  const apiBase =
    type === "purchase"
      ? `/api/contracts/${contractId}/purchases`
      : `/api/contracts/${contractId}/tasks`;

  const toggleTask = async (task: Task) => {
    setLoading(task.id);
    const key = type === "purchase" ? "toggleId" : "toggleId";
    await fetch(apiBase, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ [key]: task.id }),
    });
    setLoading(null);
    router.refresh();
  };
